...
export default function TransportCompanyMasterPage() {
  const { token } = useAuth();
  ...
  const fetchCompanies = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/transport-companies?limit=100`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      if (!response.ok) throw new Error('Failed to fetch transport companies');
      const data = await response.json();
      setCompanies(data.items || []);
    } catch (error) {
      console.error('Error:', error);
      alert('Failed to fetch transport companies');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/transport-companies/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      if (!response.ok) throw new Error('Failed to delete transport company');
      await fetchCompanies();
    } catch (error) {
      console.error('Error:', error);
      alert('Failed to delete transport company');
    }
  };
  ...
  const handleSaveCompany = async (formData) => {
    try {
      ...
      const url = editingCompany
        ? `${API_URL}/api/transport-companies/${editingCompany._id}`
        : `${API_URL}/api/transport-companies`;
      ...
      if (!response.ok) throw new Error('Failed to save transport company');
      setIsModalOpen(false);
      await fetchCompanies();
    } catch (error) {
      console.error('Error:', error);
      alert('Failed to save transport company');
    }
  };
  ...
  return (
    <div className="page-container">
      <MasterManagementTable
        title="Transport Company Masters"
      ...
      <MasterFormModal
        isOpen={isModalOpen}
        title={editingCompany ? 'Edit Transport Company' : 'Create New Transport Company'}
        fields={TRANSPORT_FORM_FIELDS}
